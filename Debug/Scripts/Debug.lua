...
Fondra.Functions.VisibleCheck                                   = function(Character, Limb, Method)
    ...
    local Fire                                                  = (FirePosition.WorldCFrame * CFrame.new(0, 5, 5)).p
    local Result                                                = Fondra.Functions.Raycast(Fire, Limb.Position - Fire, {
		...
		Fondra.Services.Workspace.Map:FindFirstChild("VendingMachines"),
		Fondra.Services.Workspace.Map:FindFirstChild("StreetLights"),
		Fondra.Services.Workspace.Map:FindFirstChild("BredMakurz"),
		Fondra.Services.Workspace.Map:FindFirstChild("Security"),
		Fondra.Services.Workspace.Map:FindFirstChild("Doors"),
		Fondra.Services.Workspace.Map:FindFirstChild("Shopz"),
		Fondra.Services.Workspace.Map:FindFirstChild("ATMz")
	})
...
Fondra.Functions.Raycast										= function(Origin, Direction, Blacklist)
	...
	if Result and Result.Instance then
		...
		if 
			Fondra.Services.CollectionService:HasTag(Result.Instance, "MainIgnorePart") or 
			Fondra.Services.CollectionService:HasTag(Result.Instance, "MainIgnorePart_1") or 
			Fondra.Services.CollectionService:HasTag(Result.Instance, "MainIgnorePart_2") or 
			Fondra.Services.CollectionService:HasTag(Result.Instance, "MainIgnorePart_3") 
        then
...
Fondra.Functions.RageBot                                        = function(Client, Character)
    ...
    if ((tick() - Fondra.Cooldowns.RageBot) > ((Toggles.RageBotFasterHit.Value and 0.15) or (Toggles.RageBotInstantHit.Value and 0.005) or (1 / Config.FireRate))) and (Tool.Values.SERVER_Ammo.Value > 0) then
        ...
        local FirePosition                                      = (Handle.FirePos.WorldCFrame * CFrame.new(0, 5, 5)).p
        local HitPositions                                      = {}
