...
var boxShadow = (function (elem, value) {
  var options = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};

  var max = !isNaN(options.max) ? options.max : 15;
  var min = !isNaN(options.min) ? options.min : 0.75;
  var scale = (max - min) * value;
  elem.style['box-shadow'] = `0 0 ${scale}px rgb(55, 126, 240)`;
  if (window.pJSDom.length > 0) {
    window.pJSDom[0].pJS.particles.move.speed = Math.max(scale, 1) * 1.5
  }
})
...
var reset$18 = function reset(elem) {
...
var Dancer = function () {
  function Dancer() {
    ...
    this.registerDance('box-shadow', boxShadow, reset$17)
    this.registerDance('image-shadow', imageShadow, reset$18)
  }
